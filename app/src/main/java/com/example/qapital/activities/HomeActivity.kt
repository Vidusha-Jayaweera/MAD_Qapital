...
//        auth = FirebaseAuth.getInstance()
        ...
        floatingActionBtn.setOnClickListener{
            val intent = Intent(this@HomeActivity, IncomeFetchingActivity::class.java)
            startActivity(intent)
        }
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    for (debtSnap in snapshot.children) {
                        val incomeData = debtSnap.getValue(IncomeModel::class.java)
                        incomeList.add(incomeData!!)
                    }
                }
                amountIncomeTemp = 0.0

                //take all amount expense, income and debts:
                for ((i) in incomeList.withIndex()){
                    amountIncomeTemp += incomeList[i].incomeAmount!!
                }
