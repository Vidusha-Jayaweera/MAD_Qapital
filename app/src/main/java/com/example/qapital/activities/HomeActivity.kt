...
//        auth = FirebaseAuth.getInstance()
        ...
        floatingActionBtn.setOnClickListener{
            val intent = Intent(this@HomeActivity, ExpenseInsertionActivity::class.java)
            startActivity(intent)
        }
...
//        tvAmountIncome.text = "$allTimeIncome"
    ...
    private fun fetchAmount() {
        ...
        val expenseList: ArrayList<ExpenseModel> = arrayListOf<ExpenseModel>()
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    for (debtSnap in snapshot.children) {
                        val expenseData = debtSnap.getValue(ExpenseModel::class.java)
                ...
                for ((i) in expenseList.withIndex()){
                    amountExpenseTemp += expenseList[i].expenseAmount!!
                }
